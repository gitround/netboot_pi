...
if os.geteuid() != 0:
    print(f"[red]this script requires root privallges. maybe try: sudo {argv[0]}[/]")
    exit(1)
...
if err != None:
    print(
        f"[red bold blink] unable to read usb devices. recived following error {err.decode()}"
    )
    exit(1)
devices: List[Dict] = []
device_id = 1
...
# create a boot partion of 512Mb
...
disk.commit()
print("[green] creating root partion [/green]")
# create root partion, using rest of disk
...
disk.commit()
print("[green]done partioning[/green]")
...
while acceptable == False:
    model_id = int(console.input("[green]Which pi will you be using this with: [/]"))
    try:
        available_pis[model_id - 1]
        acceptable = True
        if model_id - 1 < 0:
            acceptable = False
    except:
...
if available_pis[model_id - 1] in aarch_64_pis:
    choice = console.input(
        "Your pi supports the aarch64 cpu instrunction set.\nwould you like to use an aarch64 version of archlinuxarm? [Y/N]"
    )
    if choice.lower() == "y":
        aarch64 = "-aarch64"
        model_id = 1
...
if boot_umount.returncode != 0 or root_umount.returncode != 0:
    print("[red] unable to umount partions, you should umount them manuly[/]")
    exit(1)
else:
